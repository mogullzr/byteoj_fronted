...
(function (window) {
    let JSONP = (function () {
        let counter = 0, head, query, key, global = this;
    ...
    let Fx = new Class({
        initialize: function () {
            ...
            if (!props.tweens) {
                ...
                let end = typeof props.end == 'undefined' ? 1 : props.end;
                props.tweens = [[start, end]];
            }
            this.tweens[key] = props;
        }, update: function (time) {
            time = typeof time === 'number' ? time : now();
            for (let key in this.tweens) if (this.tweens.hasOwnProperty(key)) {
                if (!this.running[key]) {
                    this.tweenStart(key, time);
                    continue;
                }
                let tween = this.tweens[key];
                let tdelta = time - this.running[key].startTime;
                if (tdelta > tween.duration) {
                    this.tweenFinished(tween, key);
                    continue;
                }
                let delta = tween.transition(tdelta / tween.duration);
                let changes = [];
                for (let i = 0, t; t = tween.tweens[i]; i++) {
                    let x = delta * (t[1] - t[0]) + t[0];
                    changes.push(x);
                }
            ...
            let values = [];
            for (let i = 0, tween; tween = this.tweens[key].tweens[i]; i++)
                values.push(tween[0]);
            ...
            for (let i = 0, listener; listener = this.listeners[i]; i++)
    ...
    function each(arr, func, bindObject) {
        ...
        for (let key in arr) if (arr.hasOwnProperty(key))
    ...
    function code(name) {
        let table = {38: 'up', 40: 'down', 37: 'left', 39: 'right', 27: 'esc'};
    ...
    function getRect(element) {
        ...
        let rect = {width: element.offsetWidth, height: element.offsetHeight, left: 0, top: 0};
        let el = element;
        while (el) {
    ...
    function getScrollSize() {
        let doc = getCompatElement();
        let min = {x: doc.clientWidth, y: doc.clientHeight};
    ...
    let KickAss = new Class({
        initialize: function (options) {
            ...
            let currentTime = now();
            let tdelta = (currentTime - this.lastUpdate) / 1000;
            this.updateWindowInfo();
            for (let i = 0, player; player = this.players[i]; i++) {
            ...
            let isIEQuirks = (!!window.ActiveXObject) && document.compatMode == "BackCompat";
            this.windowSize = {
            ...
            for (let i = 0, el; el = this.elements[i]; i++) {
    ...
    let StatisticsManager = new Class({
    ...
    let Menu = new Class({
        initialize: function (game) {
            ...
            let shipId = getGlobalNamespace().KICKASSSHIPID || "";
            ...
            this.onMessage = bind(this, function (event) {
                ...
                let type = t.shift().replace(/^./g, function (match) {
            ...
            pieces = pieces.split(",");
            let shipId = pieces[0];
            let weaponId = pieces[1];
            let isInitial = pieces[2] === 'initial';
    ...
    let MenuManager = new Class({
        initialize: function (game) {
            ...
            for (let id in Weapons) if (Weapons.hasOwnProperty(id)) {
            ...
            let all = this.container.getElementsByTagName('*');
            ...
            let last = this.weaponsMenu.getElementsByTagName('li');
            ...
            let points = killed * this.game.multiplier;
    ...
    let UIManager = new Class({
        initialize: function (game) {
            ...
            if (this.pointBubbles[id]) {
                let bubble = this.pointBubbles[id];
            ...
            let id = 'bubble' + (this.UNIQID++);
            let y = this.game.scrollPos.y + pos.y;
            let bubble = newElement('span', {
                innerHTML: points,
            ...
            let top = 100;
            for (let id in this.messages) if (this.messages.hasOwnProperty(id))
                top = Math.max(this.messages[id].kickassto + this.messages[id].offsetHeight + 10, top);
            return top;
        }
    });
    let AudioManager = new Class({
        initialize: function (src, formats) {
            ...
            let TYPES = {"ogg": "audio/ogg", "mp3": "audio/mpeg"}
            let audio = document.createElement("audio");
            for (let i = 0, format; format = this.formats[i]; i++) {
    ...
    let Player = new Class({
        initialize: function (game) {
            ...
            this.ship = ship;
            this.verts = [];
            for (let i = 0, vert; vert = this.ship.points[i]; i++)
            ...
            let newCannons = [];
            for (let i = 0, cannon; cannon = this.cannons[i]; i++) {
                let newCannon = new cannonClass(this, this.game, cannon.pos.x, cannon.pos.y, cannon.angle);
                newCannons.push(newCannon);
                cannon.destroy();
            }
            this.cannons = newCannons;
        }, addThrusters: function (thrusters) {
            for (let i = 0, data; data = thrusters[i]; i++) {
                let thruster = new Thruster(data);
                this.thrusters.push(thruster);
            }
        }, addCannons: function (cannons) {
            for (let i = 0, data; data = cannons[i]; i++) {
                let weaponClass = WeaponMap[data.m] || WeaponMap.cannon;
                let cannon = new weaponClass.cannonClass(this, this.game, data.p.x, data.p.y, data.a);
                cannon.player = this;
            ...
            let frictionedAcc = this.acc.mulNew(tdelta).sub(this.vel.mulNew(tdelta * this.friction));
            this.vel.add(frictionedAcc);
            if (this.vel.len() > this.terminalVelocity)
                this.vel.setLength(this.terminalVelocity);
            let posDelta = this.vel.mulNew(tdelta);
            this.pos.add(posDelta);
            if (this.game.statisticsManager) {
                this.game.statisticsManager.increaseDistanceWithPixels(posDelta.len());
            }
            let showFlames = !this.acc.is({x: 0, y: 0});
            for (let i = 0, thruster; thruster = this.thrusters[i]; i++) {
            ...
            let w = this.game.windowSize.width;
            let h = this.game.windowSize.height;
            ...
            let w = this.game.windowSize.width;
            let h = this.game.windowSize.height;
            let rightBound = this.pos.x + this.sheet.rect.size.width / 2;
            let bottomBound = this.pos.y + this.sheet.rect.size.height / 2;
            if (rightBound > w) {
            ...
            let ret = false;
            for (let i = 0, vert; vert = this.verts[i]; i++) {
            ...
            for (let i = 0, obj; obj = this.thrusters[i]; i++) {
                let p1 = (new Vector(obj.pos.x - obj.size.width / 2, obj.pos.y - obj.size.height / 2)).rotate(obj.angle);
                let p2 = (new Vector(obj.pos.x + obj.size.width / 2, obj.pos.y - obj.size.height / 2)).rotate(obj.angle);
                let p3 = (new Vector(obj.pos.x - obj.size.width / 2, obj.pos.y + obj.size.height / 2)).rotate(obj.angle);
                let p4 = (new Vector(obj.pos.x + obj.size.width / 2, obj.pos.y + obj.size.height / 2)).rotate(obj.angle);
    ...
    let Thruster = new Class({
        initialize: function (data, ship) {
            ...
            let redWidth = this.size.width, redIncrease = this.size.width * 0.05, yellowWidth = this.size.width * 0.8,
              yellowIncrease = yellowWidth * 0.1, halfRed = redWidth / 2, halfYellow = yellowWidth / 2,
              offsetY = -this.size.height / 2, metaY = 0;
            let px = this.pos.x;
            let py = this.pos.y - this.size.height / 2;
            ...
            for (let x = 0; x < redWidth; x += redIncrease)
            ...
            for (let x = 0; x < yellowWidth; x += yellowIncrease)
            ...
            let pos = vec(px, py);
            for (let i = 0, p; p = this.flames.r[i]; i++)
                p.sub(pos).rotate(this.angle).add(pos);
            for (let i = 0, p; p = this.flames.y[i]; i++)
                p.sub(pos).rotate(this.angle).add(pos);
        }
    });
    let BombManager = new Class({
        initialize: function (game) {
            this.game = game;
            ...
            let message = this.game.ui.showMessage("3...", 5000);
            delay(1000, function () {
                message.innerHTML = "2...";
            }, this);
            delay(2000, function () {
            ...
            let index = this.game.bulletManager.enemyIndex;
            for (let i = 0, el; (el = index[i]) && i < 10; i++) {
                let rect = getRect(el);
                let center = new Vector(rect.left + rect.width / 2, rect.top + rect.height / 2);
    ...
    let ELEMENTSTHATARENOTTOBEINCLUDED = ['BR', 'SCRIPT', 'STYLE', 'TITLE', 'META', 'HEAD', 'OPTION', 'OPTGROUP', 'LINK'];
    let ELEMENTSIZETHRESHOLD = 5;
    let BulletManager = new Class({
        initialize: function (game) {
            this.game = game;
            this.lastBlink = 0;
            ...
            if (now() - this.lastBlink > 250) {
                for (let i = 0, el; el = this.enemyIndex[i]; i++) {
                    if (!this.blinkActive)
            ...
            for (let key in this.bullets) if (this.bullets.hasOwnProperty(key))
                for (let i = 0, bullet; bullet = this.bullets[key][i]; i++)
                    bullet.destroy();
            this.bullets = {};
        }
    });
    let SessionManager = new Class({
        initialize: function (game) {
            this.game = game;
    ...
    let ExplosionManager = new Class({
        initialize: function (game) {
            this.game = game;
            this.explosions = [];
        }, update: function (tdelta) {
            let time = now();
            for (let i = 0, explosion; explosion = this.explosions[i]; i++) {
            ...
            let explosion = new explosionClass(pos, forElement);
    ...
    let Cannon = new Class({
        initialize: function (player, game, x, y, angle) {
            ...
            let pos = this.getABulletPos();
            let dir = this.getABulletDir();
            let bullet = new bulletClass(pos, dir);
    ...
    let LaserCannon = new Class({
        Extends: Cannon, initialize: function (player, game, x, y, angle) {
            ...
            for (let i = 0, laser; laser = this.lasers[i]; i++) {
                laser.update(tdelta);
    ...
    let BallCannon = new Class({
        Extends: Cannon, initialize: function () {
            ...
            let time = now();
            for (let i = 0, bullet; bullet = this.bullets[i]; i++) {
            ...
            let bullet = this.createBullet(Bullet);
    ...
    let Bullet = new Class({
        initialize: function (pos, dir) {
            ...
            let element = document.elementFromPoint(this.pos.x, this.pos.y);
            if (element && element.nodeType == 3)
                element = element.parentNode;
            let didFind = element && this.game.bulletManager.isDestroyableFromCollision(element) ? element : false;
            return didFind;
        }, checkBounds: function () {
            let w = this.game.windowSize.width;
            let h = this.game.windowSize.height;
    ...
    let LaserBullet = new Class({
        Extends: Bullet, initialize: function () {
            ...
            let s = Math.max(GameGlobals.laserImage.width, GameGlobals.laserImage.height);
    ...
    let ParticleExplosion = new Class({
        Extends: Explosion, initialize: function (pos, element) {
            ...
            for (let i = 0, j = !GameGlobals.hasCanvas ? 10 : 40; i < j; i++) {
                this.particles.push({
            ...
            let right = this.sheet.rect.getRight();
            let bottom = this.sheet.rect.getBottom();
            let w = this.game.windowSize.width;
            let h = this.game.windowSize.height;
            if (right > w)
                this.pos.x -= right - w;
    ...
    let MegaParticleExplosion = new Class({
    ...
    let SplitExplosion = new Class({
        Extends: Explosion, initialize: function (pos, element) {
            ...
            let pieces = this.createClones();
            let left = pieces[0], right = pieces[1];
            let lT = 'rotate(-' + random(30, 50) + 'deg) translate(-100px, 40px)';
            let rT = 'rotate(' + random(30, 50) + 'deg) translate(100px, 40px)';
            ...
            let coords = getRect(this.element);
            let leftContainer = this.createContainer(coords);
            let rightContainer = this.createContainer(coords);
            let left = cloneElement(this.element);
            let right = cloneElement(this.element);
            addClass(left, 'KICKASSELEMENT');
            addClass(right, 'KICKASSELEMENT');
            let styles = {margin: 0, overflow: 'hidden'};
    ...
    let WeaponMap = {'cannon': Weapons[1], 'laser': Weapons[2]};
    let SheetCanvas = new Class({
    ...
    let Sheet = new Class({
        initialize: function (rect) {
            ...
            this.drawer.setStrokeColor('black');
            this.drawer.setLineWidth(1.5);
            for (let i = 1, vert, lastVert = verts[0]; vert = verts[i]; i++, lastVert = vert) {
                let o = lineOffsets[i - 1];
            ...
            for (let i = 0, particle; particle = particles[i]; i++) {
            ...
            let ret = vec.cp().sub(new Vector(this.rect.size.width / 2, this.rect.size.height / 2));
            return ret;
        }, destroy: function () {
            this.drawer.destroy();
        }
    });
    let namespace = getGlobalNamespace();

    let initKickAss = function () {
        // If an instance of KickAss is already present, we add a player
        if (!namespace.KICKASSGAME) {
            if (namespace.KICKASS_SITE_KEY) {
                let mySite = new MySite(namespace.KICKASS_SITE_KEY);
