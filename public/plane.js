...
(function (window) {
  ...
  let Fx = new Class({
    ...
    tweenFinished: function (tween, key) {
      let values = [];
      for (let i = 0, t; (t = tween.tweens[i]); i++) values.push(t[1]);
  ...
  let Player = new Class({
    ...
    addCannons: function (cannons) {
      for (let i = 0, data; (data = cannons[i]); i++) {
        ...
        let cannon = new weaponClass.cannonClass(
          this,
          this.game,
          data.p.x,
          data.p.y,
    ...
    generateFlames: function () {
      ...
      for (let x = 0; x < redWidth; x += redIncrease)
        this.flames.r.push(
          vec(
            x - halfRed + px,
            this.flameY * random(this.size.height * 0.7, this.size.height) + py
          )
        );
  ...
  let ELEMENTSTHATARENOTTOBEINCLUDED = [
    "BR",
    "SCRIPT",
    "STYLE",
    "TITLE",
