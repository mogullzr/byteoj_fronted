...
const currentProblemGroup = ref(0); // 当前题目分组索引
...
const PageClick = async (Page: number | any) => {
  ...
  if (res.code === 0) {
    ...
    currentProblemGroup.value = 0; // 重置题目分组
  }
};
...
const problemGroups = computed(() => {
  const groups = [];
  for (let i = 0; i < problem_list.value.length; i += 3) {
    groups.push(problem_list.value.slice(i, i + 3));
  }
  return groups;
});

// 切换题目分组
const toggleProblemGroup = () => {
  currentProblemGroup.value =
    (currentProblemGroup.value + 1) % problemGroups.value.length;
};
      ...
      <table class="ranking-table">
        ...
        <thead>
          <tr>
            ...
            <th class="user-col">参赛用户</th>
            <th class="school-col">学校</th>
            <th class="ac-col">通过数</th>
            <th class="penalty-col" v-if="status == 1 || status == 2">
              <div v-if="status == 0">总分</div>
              <div v-else>罚时</div>
            </th>
            <th
              class="problem-col"
              v-for="(problem, index) in problemGroups[currentProblemGroup]"
              :key="problem.problem_id"
              :class="{
                'last-in-group': index === 2 && problemGroups.length > 1,
              }"
            >
              <router-link
                :to="
              ...
              <button
                v-if="
                  (index === 2 && problemGroups.length > 1) ||
                  problemGroups.length != 3
                "
                class="toggle-group-btn"
                @click="toggleProblemGroup"
              >
                <svg
        ...
        <tbody v-if="status == 0">
          ...
          >
            ...
            <td
              ...
              v-for="record in user.problem_record_list.slice(
                currentProblemGroup * 3,
                currentProblemGroup * 3 + 3
              )"
