...
<template>
  ...
  <div v-for="data in dataList" class="post-item">
    ...
    <div class="flex mt-4">
      ...
      <div class="flex-1">
        <div class="font-bold">
          ...
          <span
            v-html="highlightKeyword(data.chinese_name, <string>searchRequest.keyword)"
          ></span>
        </div>
    ...
    <div class="mx-6 my-2 flex">
      <div class="mx-4">
        <div class="mx-8 text-lg" v-if="data.algorithm_tags?.length !== 0">
          ...
          >
            <span
              ...
              class="badge badge-lg border-2 font-bold text-red-500"
              style="background-color: #f4f4f4"
              >{{ tag }}</span
            >
            <span
              v-else
              class="badge badge-lg border-2 font-bold text-gray-500"
              style="background-color: #f4f4f4"
              >{{ tag }}</span
            >
          </button>
        ...
        <div class="mx-8 my-2 text-lg" v-if="data.source && data.source !== ''">
          ...
          <button class="mx-1" @click="searchBySource(data.source)">
            <span
              v-if="searchRequest.sourceList?.includes(data.source)"
              class="badge badge-lg border-2 font-bold text-red-500"
              style="background-color: #f4f4f4"
              >{{ data.source }}</span
            >
    ...
    <div class="hover-button">
      <router-link
        :to="'/problems/algorithm/' + data.problem_id"
