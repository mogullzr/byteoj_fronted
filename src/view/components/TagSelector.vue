...
const availableTags = computed(() => {
  return props.allTags.filter(tag =>
      !selectedTags.value.some(selected => selected.tag_id === tag.tag_id)
  )
})
...
const filteredTags = computed(() => {
  return availableTags.value.filter(tag =>
      tag.tag_name.toLowerCase().includes(searchQuery.value.toLowerCase())
  )
})
...
const removeTag = (tag: Tag) => {
  selectedTags.value = selectedTags.value.filter(t => t.tag_id !== tag.tag_id)
  emit('update:selectedTags', selectedTags.value)
}
</script>
