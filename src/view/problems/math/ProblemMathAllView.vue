...
<template>
  <div class="card-body mt-4 h-30 bg-base-100 shadow-xl rounded-box">
    <div class="w-7/12 m-auto">
      <div class="font-bold text-center text-4xl">ByteOJ数学题库</div>
      ...
      <table class="table" v-if="problem_list.length !== 0">
        ...
        >
          ...
          <router-link
              ...
              :to="'/problems/other/' + problem.problem_id"
          >
            <td
                v-html="highlightKeyword(problem.problem_name, <string>searchRequest.keyword)"
            ></td>
          </router-link>
          <td class="w-64">
            <button
          ...
          <td>
            <span class="badge badge-ghost">{{ problem.source_name }}</span>
          </td>
          <td>
            <button
                @click="
